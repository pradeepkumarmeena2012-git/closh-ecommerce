...
export const handleCompleteDelivery = asyncHandler(async (req, res) => {
    ...
    // Re-enable rider availability since they have completed the order (only if fully done)
    let updatedRider;
    if (!hasRejectedItems) {
        updatedRider = await DeliveryBoy.findByIdAndUpdate(order.deliveryBoyId, { status: 'available' }, { new: true }).select('availableBalance totalEarnings totalDeliveries');
